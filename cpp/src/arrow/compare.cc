  ...
  Status CompareListView(const TypeClass& type) {
    ...
    auto compare_view = [&](int64_t i, int64_t length) -> bool {
      for (int64_t j = i; j < i + length; ++j) {
        ...
        RangeDataEqualsImpl impl(
            options_, floating_approximate_, left_values, right_values,
            left_.offset + left_start_idx_ + left_offsets[j],
            right_.offset + right_start_idx_ + right_offsets[j], size);
        if (!impl.Compare()) {
          return false;
        }
      }
...
class TypeEqualsVisitor {
 public:
  ...
  template <typename T>
  enable_if_t<is_list_view_like_type<T>::value, Status> Visit(const T& left) {

...
Status ConcatenateListViewOffsets(const BufferVector& buffers,
  ...
  offset_type num_child_values = 0;
  int64_t elements_length = 0;
  for (size_t i = 0; i < buffers.size(); ++i) {
    const auto displacement = num_child_values - value_ranges[i].offset;
    ...
    num_child_values += static_cast<offset_type>(value_ranges[i].length);
  }

...
struct BitmapTag {
  enum Value {
    kMustCheck,  // bitmap pointer and buffer values must be checked at runtime
    kChecked,    // bitmap pointer is known to be present
...
class OptionalValidity {
 ...
 public:
  ...
  static constexpr bool kMustCheckBitmap = Tag == BitmapTag::kMustCheck;
  static constexpr bool kCheckedBitmap = Tag == BitmapTag::kChecked;
  static constexpr bool kEmptyBitmap = Tag == BitmapTag::kEmpty;
  ...
  OptionalValidity(int64_t length, const uint8_t* bitmap,
      ...
      : length(length), bitmap(bitmap), null_count(null_count), offset(offset) {
    if constexpr (Tag == BitmapTag::kChecked) {
      ...
      this->null_count = 0;
    } else if constexpr (Tag == BitmapTag::kMustCheck) {
  ...
  ARROW_FORCE_INLINE bool CanCast() const {
  ...
  template <BitmapTag::Value To>
  ARROW_FORCE_INLINE OptionalValidity<To>* TryCast() && {
    if constexpr (BitmapTag::PartialOrd<Tag, To>::lte) {
  ...
  ARROW_FORCE_INLINE std::enable_if_t<IsProfitableOverride<UnsafeTagOverride, Tag>, bool>
  ...
  template <BitmapTag::Value UnsafeTagOverride = Tag>
  ARROW_FORCE_INLINE bool IsNull(int64_t i) const {
